...
logger = logging.getLogger("pipeline.ingest.ziprecruiter")
...
def fetch_ziprecruiter_jobs(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    if not bool(config.get("ZIPRECRUITER_ENABLED", False)):
        logger.info("pipeline.ingest.ziprecruiter.disabled")
        return []

    api_url = ensure_str(config.get("ZIPRECRUITER_API_URL"))
    api_key = ensure_str(config.get("ZIPRECRUITER_API_KEY"))
    if not api_url or not api_key:
        logger.warning("pipeline.ingest.ziprecruiter.misconfigured")
        return []
    ...
    for job in raw_jobs or []:
        ...
        if not title or not job_url:
            logger.debug("pipeline.ingest.ziprecruiter.malformed", extra={"reason": "missing title or url"})
            continue
    ...
    logger.info("pipeline.ingest.ziprecruiter.success", extra={"count": len(out)})
    return out
