...
logger = logging.getLogger("pipeline.ingest.glassdoor")
...
def fetch_glassdoor_jobs(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    if not bool(config.get("GLASSDOOR_ENABLED", False)):
        logger.info("pipeline.ingest.glassdoor.disabled")
        return []

    api_url = ensure_str(config.get("GLASSDOOR_API_URL"))
    api_key = ensure_str(config.get("GLASSDOOR_API_KEY"))
    if not api_url or not api_key:
        logger.warning("pipeline.ingest.glassdoor.misconfigured")
        return []
    ...
    for job in raw_jobs or []:
        ...
        if not title or not job_url:
            logger.debug("pipeline.ingest.glassdoor.malformed", extra={"reason": "missing title or url"})
            continue
    ...
    logger.info("pipeline.ingest.glassdoor.success", extra={"count": len(out)})
    return out
