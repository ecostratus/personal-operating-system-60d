...
- Filters & Sources: Adopt shared helpers for consistent input handling and typesafety.
...
- Enrichment config lists (seniority patterns, stack patterns, keyword lists, rolefit heuristics) are normalized once at load time using pure, typed functions.
- Function entry points defensively normalize inputs to preserve semantics and repeatable behavior.

## Enrichment Behavior
- Semantics, scoring, thresholds, and rolefit heuristics remain unchanged.
...
- Validates:
  - Edgecase handling (mixed case, punctuation, whitespace, None inputs)
...
- Enrichment
  ...
  - [x] Remotefriendly flags processed through normalized aliases
